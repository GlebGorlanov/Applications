<template>
    <div class="volumeWindow rounded-[3.2vw]">
        ...
        <div class="content">
            ...
            <div class="volumeWrap rounded-[2.3vw]" v-show="volumeWrapWindow">
                <output @click="switchVolume(value)" class="sound rounded-[2.5vw]"><img class="image w-[55%]" :src="getImg(soundImg)" alt="sound"></output>
                <output class="range"><input class="slider" type="range" min="0" max="30" v-model="value"></output>
                <output class="outputNumber text-[7vw]">{{ value }}</output>
            </div>
            <!------------------------------------------ Элементы навигационного меню -------------------------------------------------->
            <div class="iconsInfo" v-show="iconsInfoWindow">
                <div class="btnInfo rounded-[3vw]" @click="hideIconsInfo('m')">
